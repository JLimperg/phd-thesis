...
The same cannot be said for regular, non-computerised mathematics~\cite{Voevodsky14,Hales08} or for many of the automated systems that aim to verify properties of computer programs~\cite{CSmith,YinYang,OpFuzz,DeltaDebugSMT,HistFuzz,STORM}.
...
Whenever we apply a rule with success probability $p_r$ to a parent goal with priority $p_g$, the priority of each subgoal is $p_g p_r$.
...
For example, the goal $\Ex{n : ℕ}{0 < n ∧ n < 2}$ is decomposed by built-in Aesop rules into two goals $0 < \mv{n}$ and $\mv{n} < 2$, where $\mv{n}$ is a newly created metavariable.
...
Outside Mathlib, Aesop is used in various mathematical formalisation projects, e.g.\ the Equational Theories project \cite{EquationalTheories} and the formalisation of a proof of the polynomial Freiman-Ruzsa conjecture\footnote{\url{https://github.com/teorth/pfr}}.
...
I have added Sec.~\ref{sec:forward:rpinf}, which discusses an attempt at performance optimisation that is not included in the final implementation.
