...
In this thesis, we introduce \emph{Aesop} (Automatic Extensible Search for Obvious Proofs), a tactic for the Lean~4 proof assistant.
...
We therefore added a script optimiser to Aesop that addresses these challenges (among others).
...
We propose two techniques to address this issue: a static method that misses some opportunities for reordering, and a dynamic method that reorders as much as possible.
...
For a pragmatic, heuristic tool like Aesop, the ultimate metric of success is adoption, so it is my pleasure to report that Aesop has been used for a variety of purposes throughout the Lean ecosystem.
