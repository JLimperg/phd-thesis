...
The same cannot be said for regular, non-computerised mathematics~\cite{Voevodsky14,Hales08} or for many of the automated systems that aim to verify various properties of computer programs~\cite{CSmith}.
...
Moreover, a promising line of research has recently started to combine ITPs with machine learning, aiming to mitigate the unreliability of large language models by checking their outputs with ITPs~\cite{Lample22,LeanDojo,GoedelProver,DeepSeekProver,Aristotle}.
...
This large mathematics library for Lean~4 is perhaps the first coherent library of mathematics formalised in an interactive theorem prover and primarily developed by mathematicians rather than computer scientists.
...
Usually, Aesop is provided not with exactly the rules needed for a proof, but rather with a superset designed to prove a large class of simple goals.
...
Essentially the same setup is used by various other proof search tactics, particularly Rocq's and Isabelle's auto tactics~\cite{Rocq,IsabelleAuto}.
Aesop distinguishes itself by three features, summarised in the next three sections and detailed in Chapters~\ref{chap:aesop,chap:forward,chap:script}.
...
This approach is used in HyperTree Proof Search (HTPS)~\cite{Lample22}, an LLM-based theorem proving methodology.
...
(TODO example?)
Aesop has been used as a backend for HTPS-style systems such as ABEL~\cite{ABEL} and LeanDojo~\cite{LeanDojo}.
...
For a more natural evaluation, we adapt the benchmark used by lean-auto~\cite{lean-auto} to run different Aesop configurations on Mathlib theorems.
...
A notable heuristic prover based not on tree search but saturation is Isabelle's \emph{auto2}~\cite{auto2}.
...
By contrast, \emph{automated theorem provers} (ATPs), including SAT solvers~\cite{SATHandbook}, SMT solvers~\cite{Z3,CVC5} and superposition-based theorem provers~\cite{Vampire,Zipperposition}, provide \enquote{push-button} automation that seeks to find proofs without any help from users.
...
Examples of this approach are Isabelle's \emph{Metis}~\cite{Metis}, Rocq's \emph{sauto}~\cite{sauto}, Agda's \emph{Agsy}~\cite{Agsy} and Lean's \emph{Duper}~\cite{Duper}, \emph{Canonical}~\cite{Canonical} and \emph{grind}\footnote{\url{https://lean-lang.org/doc/reference/latest/find/?domain=Verso.Genre.Manual.section&name=grind-tactic}}.
However, these ATPs tend to be somewhat weak since they must be re-implemented for each ITP and are therefore less mature than established ATPs such as Z3~\cite{Z3}, CVC5~\cite{CVC5}, Vampire~\cite{Vampire} or Zipperposition~\cite{Zipperposition}.
...
The main alternative to native ATPs are external ATPs integrated via \emph{hammers} such as Isabelle's \emph{Sledgehammer}~\cite{Sledgehammer}, Rocq's \emph{CoqHammer}~\cite{CoqHammer} and Lean's \emph{LeanHammer}~\cite{LeanHammer}.
...
More recently, some ATPs have gained the ability to report detailed proofs (TODO refs: Alethe? TSTP?), which can directly be translated back into the ITP's logic.

Yet another style of ATP integration is exemplified by the Dafny~\cite{Dafny}, \Fstar~\cite{Fstar}, Verus~\cite{Verus} and Why3~\cite{Why3} tools, which are deeply integrated with an SMT solver and allow users to provide only a coarse proof sketch, with the SMT solver filling in most of the details.
...
Various architectures for such NTPs have been proposed, including one-shot NTPs, where the LLM directly generates an entire proof~\cite{DeepSeekProver,GoedelProver,GoedelProverV2}; tactic-by-tactic NTPs, where LLMs generate individual tactics to form a large search tree similar to Aesop's~\cite{Lample22,LeanDojo,ABEL,Aristotle,AlphaProof,DeepSeekProverV15}; decomposition-based systems that ask LLMs to recursively decompose the problem statement into smaller lemmas until the lemmas can be directly proved~\cite{DSP,Hilbert,SeedProver}; and free-form agents that let the LLM control the search process\footnote{E.g.\ OpenGauss (\url{https://www.math.inc/opengauss}) or Leanstral (\url{https://mistral.ai/news/leanstral}).}.
...
For instance, several systems have achieved gold and silver medals at the 2024 and 2025 International Mathematical Olympiads.
...
Even one one-shot attempt is typically much more expensive than an ATP invocation since LLMs of sufficient size are slow and require costly hardware, and systems based on tactic-level search or decomposition tend to involve many more LLM queries.
...
Recent versions of Mathlib call Aesop around 7000 times, often through wrapper tactics that call Aesop with a specialised rule set.
...
Geoffrey Irving's \tac{interval} tactic%
\footnote{\url{https://github.com/girving/interval}} for interval arithmetic with floating point numbers is largely a wrapper around Aesop with a particular rule set.
Aeneas~\cite{Aeneas} uses a custom implementation of forward rules, inspired by Aesop's, in its proof automation.
...
The Pantograph library~\cite{Pantograph}, which aims to provide a foundation for tactic-by-tactic NTPs, handles metavariables in a manner inspired by Aesop.
