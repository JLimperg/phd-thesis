...
I conjecture that this approach is complete, in the sense that if there is a sequence of rule applications that will solve a given goal, a fair search will find it.

As an additional optimisation, we partition sets of related goals into \emph{metavariable clusters}, which are minimal sets of goals transitively coupled by metavariables.
...
Other heuristic, customisable provers include ACL2's \emph{waterfall}, PVS's \emph{grind} and, more distantly related, Mizar's \emph{by}.
...
More recently, some ATPs have gained the ability to report detailed proofs~\cite{Alethe,CVC5ProofVerification,CVC5RewriteVerification}, which can directly be translated back into the ITP's logic.
...
Even a single one-shot attempt is typically more expensive than an ATP invocation since LLMs of sufficient size are slow and require costly hardware.
Systems based on tactic-level search or decomposition tend to involve large numbers of LLM queries.
...
Outside of Mathlib, Aesop is used in various mathematical formalisation projects, e.g.\ the Equational Theories project \cite{EquationalTheories} and the formalisation of a proof of the polynomial Freiman-Ruzsa conjecture\footnote{https://github.com/teorth/pfr}.
