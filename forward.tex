% TODO integrate changes from latest paper version
...
Until recently, Aesop used what I call the \emph{naive algorithm} to apply forward rules.
...
Our evaluation (Sec.~\ref{sec:forward:evaluation}) shows that this new implementation of forward reasoning in Aesop substantially outperforms the previous, naive implementation on synthetic benchmarks.
...
This chapter is closely based on the paper \emph{Incremental Forward Reasoning for White-Box Proof Search}~\cite{Forward}, which is joint work with Xavier Généreux.
...
What exactly \enquote{matching} means is discussed in Sec.~\ref{sec:forward:implementation}.
...
This is an important optimisation: it prevents us from constructing $r~a~b$ in two different but ultimately redundant ways.
...
Our first benchmark is a scaled-up variant of the transitivity example from Sec.~\ref{sec:forward:incremental}.
