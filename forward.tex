...
Until recently, Aesop used the \emph{naive algorithm} from the previous chapter to apply forward rules.
...
First, observe that complete matches can only be generated once all slots have been filled.
...
Second, observe that the variable indices are only queried for hypotheses at a slot $i > 1$ when a match is added at slot $i - 1$.
Hence, we delay the insertion of hypotheses at $i$ until there is at least one match in slot $i - 1$.

\para{Phased Insertion}
We delay insertions yet further by taking advantage of Aesop's partitioning of rules into several \emph{phases}.
When Aesop tries to solve a goal, it first normalises it, using only rules tagged with the \emph{normalisation} phase.
Then it tries to apply rules from the \emph{safe} phase, and only when all of them fail does it consider rules from the \emph{unsafe} phase.
...
Both implementations benefit from this, but incremental forward reasoning does so disproportionately, perhaps because it uses more custom code that, unlike Lean's built-in routines, is usually interpreted.
